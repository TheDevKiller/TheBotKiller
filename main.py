...
async def on_message(message): # Dès qu'il y a un message
	...
	if client.user.mentioned_in(message) and message.author == thedevkiller: # Changement du préfixe
		...
		if jeuJoueur == jeuBot:                                 # La deuxième personne
			...
			await client.send_message(message.channel, "Entre un élément valide à la prochaine partie s'il te plaît :win$

		await client.send_message(message.channel, embed=discord.Embed(title="Résultat du Shifumi entre " + message.author.n$
